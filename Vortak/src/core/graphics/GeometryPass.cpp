...
namespace Vortak {
    void GeometryPass::build(
        ...
        if (!mInitialized) {
            mCachedPipeline.vertexShader = graphicsDevice->createShader("res/shader/common_vertex.glsl", ShaderType::VERTEX);
            mCachedPipeline.fragmentShader = graphicsDevice->createShader("res/shader/common_fragment.glsl", ShaderType::FRAGMENT);
